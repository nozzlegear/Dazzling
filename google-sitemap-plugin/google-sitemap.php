...
function google_sitemap() {
  ...
  foreach ( $posts as $post ) {
    ...
    $url = str_replace("&", "&amp;", get_permalink( $post->ID ));

    // This plugin likes to add a bunch of "peteternal.com/blogs/news/product_variation" URLs which don't exist and return 404.
    if (preg_match('/.*peteternal.com\/blogs\/news\/product_variation\/?(?:.*)?/i', $url)) {
        continue;
    }
